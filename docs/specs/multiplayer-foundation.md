...
这样洞府升级、突破、招募/升级修士、研究功法、锻造法宝、服用丹药和寻道灵装会真实改变地图上的胜负，不再存在“本地战力一套、多人战区另一套”的分叉。每个 mutation 在 PostgreSQL 事务内读取全局世界行并加排他锁，领域函数成功后才更新 revision 和提交；HTTP 响应也在事务提交后发送。这样多个 API 进程不会同时覆盖行军、好友和占领结果。JSON fallback 也会串行执行完整 mutation 与落盘，适合开发和模拟脚本；正式部署仍必须使用 Compose 或托管 PostgreSQL。

客户端轮询会合并重叠的刷新请求，并在改名、出征、驻防、好友等 mutation 返回前暂缓刷新，避免旧快照覆盖刚完成的操作。旧版立即结算攻击接口同样先扣除完整出战编队，胜利后的存活部队进入据点守军，不会同时留在个人兵力中。
